...
bitflags::bitflags! {
    #[repr(transparent)]
    pub struct NeoMaterialFlags: u32 {
        const TEXTURE           = 1 << 0;
        const NORMAL            = 1 << 1;
        const NONE              = 0;
...
/// The GPU representation of the uniform data of a [`NeoMaterial`].
#[derive(Clone, Default, ShaderType)]
pub struct NeoMaterialUniform {
    pub color: Vec4,
    pub flags: u32,
}

impl AsBindGroupShaderType<NeoMaterialUniform> for NeoMaterial {
    fn as_bind_group_shader_type(&self, _images: &RenderAssets<GpuImage>) -> NeoMaterialUniform {
        let mut flags = NeoMaterialFlags::NONE;
        if self.texture.is_some() {
            flags |= NeoMaterialFlags::TEXTURE;
        }

        if self.normal.is_some() {
            flags |= NeoMaterialFlags::NORMAL;
        }
...
/// A component bundle for entities with a [`Mesh2dHandle`](crate::Mesh2dHandle) and a [`NeoMaterial`].
